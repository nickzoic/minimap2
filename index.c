...
mm_idx_t *mm_idx_load(FILE *fp)
{
	...
	for (i = 0; i < mi->n_seq; ++i) {
		...
		if (fread(&l, 1, 1, fp) != 1) goto fail;
		if (l) {
			s->name = (char*)kmalloc(mi->km, l + 1);
			if (fread(s->name, 1, l, fp) != l) goto fail;
			s->name[l] = 0;
		}
		if (fread(&s->len, 4, 1, fp) != 1) goto fail;
		s->offset = sum_len;
		s->is_alt = 0;
		sum_len += s->len;
	}
	for (i = 0; i < 1<<mi->b; ++i) {
		...
		khint_t k;
		idxhash_t *h;
		if (fread(&b->n, 4, 1, fp) != 1) goto fail;
		b->p = (uint64_t*)malloc(b->n * 8);
		if (fread(b->p, 8, b->n, fp) != b->n) goto fail;
		if (fread(&size, 4, 1, fp) != 1) goto fail;
		if (size == 0) continue;
		b->h = h = kh_init(idx);
	...
	if (!(mi->flag & MM_I_NO_SEQ)) {
		...
		if (fread(mi->S, 4, (sum_len + 7) / 8, fp) != (sum_len + 7) / 8) goto fail;
	}
	return mi;
